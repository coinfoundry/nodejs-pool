...
let pubSock = null;
let pubFlags = config.publisherJson ? 1 : 2;  // WireFormat.Json or WireFormat.ProtoBuf
let pubFlagsBuf = null;
...
if (cluster.isMaster) {
    ...
    pubFlagsBuf = Buffer.allocUnsafe(4);
    pubFlagsBuf.writeUInt32BE(pubFlags, 0);

    if(config.publisherRawServerSecret) {
        pubSock.curve_secretkey = new Buffer(config.publisherRawServerSecret, "hex");
        pubSock.curve_server = 1;
    
        console.log('Enabled ZMQ Curve server mode');
    }
...
function retargetMiners() {
    ...
    Object.keys(global.config.ports).forEach(function (port) { minerCount[port] = 0; });
...
function process_rpc_template(rpc_template, coin, port) {
...
function templateUpdateReal(coin) {
    global.coinFuncs.getBlockTemplate(function (rpcResponse) {
        if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
            const rpc_template = rpcResponse.result;
            const template = process_rpc_template(rpc_template, coin, global.config.daemon.port);
            console.log(threadName + "New block template found at " + rpc_template.height + " height");
            ...
            console.error("Block template request failed for port.");
            setTimeout(templateUpdateReal, 3000, coin);
        }
    });
}
...
function templateUpdate(coin, repeating) {
    global.coinFuncs.getLastBlockHeader(function (err, body) {
        if (err === null) {
            if (body.hash !== lastBlockHash[coin]) {
                lastBlockHash[coin] = body.hash;
                templateUpdateReal(coin);
            } 

            if (repeating === true) {
                setTimeout(templateUpdate, 50, coin, repeating);
            }
        } else {
            //failedPortLastBlockHeaderCoinHashFactor[coin] = coinHashFactor;
            console.error(threadName + "Last block header request for failed!");
...
function setNewBlockTemplate(template) {
    ...
    if (cluster.isMaster) {
        ...
        console.log('[*] New ' + coin_str + 'block to mine at ' + template.height + ' height with ' + template.difficulty + ' difficulty and ' + template.port + ' port');
    } else {
...
function recordShareData(miner, job, shareDiff, isBlockCandidate, hashHex, isTrustedShare, blockTemplate) {
    ...
    var share = {
        ...
        networkDifficulty: job.rewarded_difficulty,
    ...
    process.send({type: 'share', data: share});

    console.log("Accepted share at difficulty: " + job.difficulty + "/" + shareDiff + " from: " + miner.logString);
}
...
function getShareBuffer(miner, job, blockTemplate, params) {
    try {
        ...
        console.log("FYI: Can't constructNewBlob", err_str);
        return null;
    }
}
        ...
        if (rpcResult.error) {
            ...
            if (isNotifyAdmin) setTimeout(function() { // only alert if block height is not changed in the nearest time
                global.coinFuncs.getLastBlockHeader(function(err, body) {
                    ...
                    if (blockTemplate.height == body.height + 1) console.log(global.config.general.adminEmail,
            ...
            if (isRetrySubmitBlock) {
                ...
                console.log("FYI: Can't submit block to deamon on " + blockTemplate.port + " port",
                    "Input: " + shareBuffer.toString('hex') + "\n" +
                    "The pool server: " + global.config.hostname + " can't submit block to deamon on " + blockTemplate.port + " port\n" +
...
function processShare(miner, job, blockTemplate, params) {
    ...
    if (is_mm && hashDiff.ge(blockTemplate.child_template.difficulty)) { // Submit child block to the RPC Daemon.
        ...
        try {
            ...
            console.error(err_str);
            console.log("FYI: Can't construct_mm_child_block_blob", err_str);
            return invalid_share(miner);
        }
...
if (cluster.isMaster) {
    ...
    async.each(Object.keys(global.config.ports), function (port) {
        ...
        if ('ssl' in portData && portData.ssl === true) {
            ...
            server.listen(port, global.config.bind_ip, function (error) {
                if (error) {
                    console.error(threadName + "Unable to start server on: " + port + " Message: " + error);
                    return;
                }
                console.log(threadName + "Started server on port: " + port);
            });
            server.on('error', function (error) {
                console.error("Can't bind server to " + port + " SSL port!");
            });
        } else {
            ...
            server.on('error', function (error) {
                console.error("Can't bind server to " + port + " port!");
            });
        }
    });
}
