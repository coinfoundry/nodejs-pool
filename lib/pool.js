...
function templateUpdateInternal(rpcResponse, repeating) {
    if (rpcResponse && typeof rpcResponse.result !== 'undefined') {
        ...
        if (repeating !== false && global.config.pool.blockRefreshInterval) {
            setTimeout(templateUpdate, global.config.pool.blockRefreshInterval);
        }
    } else {
        setTimeout(templateUpdate, global.config.pool.blockRefreshInterval || 300, false);
    }
}
...
function templateUpdate(repeating) {
    ...
    global.coinFuncs.getBlockTemplate(global.config.pool.address, function (rpcResponse) {
        ...
        templateUpdateInternal(rpcResponse, repeating);
    });
}
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent) {
    ...
    if (this.paymentID && this.paymentID.length !== 64) {
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate) {
    ...
    const rawMsg = config.publisherJson ? JSON.stringify(share) :
        ShareMessage.encode(ShareMessage.create(share)).finish();
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    switch (method) {
        case 'login':
            console.log("Login from " + ip + ": " + params.agent);
...
const btStreaming = typeof global.config.pool.btStream === "object";
...
if(btStreaming) {
    ...
    subSock.connect(global.config.pool.btStream.url);
    subSock.subscribe(global.config.pool.btStream.topic);

    console.log('Connected to ZMQ BlockTemplate Stream ' + global.config.pool.btStream.url + "/" + global.config.pool.btStream.topic);

    subSock.on('message', function(topic, flags, message) {
        templateUpdateInternal(JSON.parse(message));
    });
} else if(typeof global.config.pool.zmqPubSocket === "string") {
    var subSock = zmq.socket('sub');
    subSock.connect(global.config.pool.zmqPubSocket);
    subSock.subscribe(global.config.pool.zmqPubTopic);

    console.log('Connected to daemon ZMQ block notify socket ' + global.config.pool.zmqPubSocket + "/" + global.config.pool.zmqPubTopic);

    subSock.on('message', function(topic, message) {
        let currentHeight = activeBlockTemplate ? activeBlockTemplate.height : 0;
        let newHeight = parseInt(message.toString(), 10);

        if(!isUpdatingTemplate && currentHeight < newHeight) {
            console.log('Block notification via ZMQ subscription for height ' + newHeight);
            templateUpdate(false);
        } else {
            debug('Ignored Block notification via ZMQ subscription');
        }
    });
} else {
    console.warn('Block template ZMQ subscription has been disabled');
}
...
async.each(Object.keys(global.config.ports), function (port) {
    ...
    if ('ssl' in portData && portData.ssl === true) {
        ...
        }, socketConn).listen(port, portData.listenAddress, function (error) {
            if (error) {
