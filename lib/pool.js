...
if(typeof global.config.pool.zmqPubSocket === "string") {
    ...
    subSock.on('message', function(topic, message) {
        let prev_hash = activeBlockTemplate ? activeBlockTemplate.previous_hash.toString("hex"): "";
        let prev_hash_new = message.toString("hex");

        console.log('Block notification via ZMQ subscription');
        templateUpdate(false);

        // if(prev_hash !== prev_hash_new) {
        //     console.log('Block notification via ZMQ subscription');
        //     templateUpdate(false);
        // } else {
        //     console.log('** Ignored Block notification via ZMQ subscription');
        // }
    });
} else {
