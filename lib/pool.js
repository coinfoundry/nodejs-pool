...
console.log('Bound publisher ZMQ socket ' + config.publisherSocket);
...
function recordShareData(miner, job, shareDiff, blockCandidate, hashHex, shareType, blockTemplate) {
    ...
    // publish
    pubSock.send([config.publisherTopic, JSON.stringify(share)]);
...
if(typeof global.config.pool.zmqPubSocket === "string") {
