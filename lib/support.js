...
function jsonRequest(host, port, data, callback, path, ssl) {
    path = path || 'json_rpc';
    let uri;
    if (ssl) {
        uri = "https://" + host + ":" + port + "/";
    ...
    if(global.config.daemon.user != null && global.config.daemon.password != null)
        client.setBasicAuth(global.config.daemon.user, global.config.daemon.password);
...
function rpc(host, port, method, params, callback, path, ssl) {
    ...
    return jsonRequest(host, port, data, callback, path, ssl);
}
...
module.exports = function () {
    return {
        rpcDaemon: function (method, params, callback) {
            rpc(global.config.daemon.address, global.config.daemon.port, method, params, callback, global.config.daemon.httpPath, global.config.daemon.ssl);
        },
        rpcWallet: function (method, params, callback) {
            rpc(global.config.wallet.address, global.config.wallet.port, method, params, callback, global.config.wallet.httpPath, global.config.daemon.ssl);
        },
