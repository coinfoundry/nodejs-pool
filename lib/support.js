...
function rpc(host, port, method, params, callback, timeout) {
    ...
    return jsonRequest(host, port, data, false, callback, global.config.daemon.httpPath || 'json_rpc', timeout);
}
...
function rpc_wallet(host, port, method, params, callback) {
    ...
    return jsonRequest(host, port, data, true, callback, global.config.daemon.httpPath || 'json_rpc', 30*60*1000);
}
